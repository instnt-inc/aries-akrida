...
class AcapyIssuer(BaseIssuer):
    ...
    def get_invite(self, out_of_band=False):
        ...
        if out_of_band:
            ...
            r = requests.post(
                ...
                json={"metadata": {"transaction_id": "shivam"}},
                headers=headers,
            )
    ...
    def revoke_credential(self, connection_id, credential_exchange_id):
        ...
        print("IssuerAgent_revoke_credential() request body -->", r.request.body)
        print("IssuerAgent_revoke_credential() status code -->", r.status_code)
        print("IssuerAgent_revoke_credential() response body -->", r.json())
        if r.status_code != 200:
            raise Exception(r.content)
    ...
    def send_message(self, connection_id, msg):
        ...
        print("IssuerAgent_send_message() request body -->", r.request.body)
        print("IssuerAgent_send_message() status code -->", r.status_code)
        print("IssuerAgent_send_message() response body -->", r.json())
        r = r.json()
